...
public class purchase_services {
	...
	public Book purchase_book(int id) {
		try {
		Book book =restTemplate.getForObject("http://192.168.56.1:8080/BookItem/"+id, Book.class);
		if(book.getStock()>0)
		{
			book.setStock(book.getStock()-1);
			 restTemplate.put("http://192.168.56.1:8080/Book/"+id+"/stock/dec/0",Book.class);
			 return book;
		}
		else return null;
	}
